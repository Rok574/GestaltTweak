...
struct GestaltNotice: Identifiable {
    enum Kind { case success, error, backupCreated, riskWarning }
    ...
    var title: String {
        ...
        case .backupCreated: "Backup Complete"
        case .riskWarning: "High Risk"
        }
    }
}
...
struct PlistValueInfo {
    ...
    static func info(for value: Any?) -> PlistValueInfo {
        ...
        case .integer, .float, .boolean:
            summary = text
        case .data:
            summary = String(format: "Data (%d bytes)", (value as? Data)?.count ?? 0)
        case .array:
            summary = String(format: "Array (%d items)", (value as? NSArray)?.count ?? 0)
        case .dictionary:
            summary = String(format: "Dictionary (%d items)", (value as? NSDictionary)?.count ?? 0)
        }
    ...
    private static func jsonObject<T>(from text: String, expected: T.Type) throws -> Any {
        guard let data = text.data(using: .utf8),
            ...
            throw PlistValueError.invalid(
                String(format: "Invalid JSON for type %@", String(describing: T.self))
            )
        }
        return object
    }
